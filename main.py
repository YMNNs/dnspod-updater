...
def main():
    ...
    try:
        ...
        dnspod.log('ERROR', '文件config.json不存在或可能已经损坏')
        dnspod.exit_after_countdown()
    try:
        headers, params, domains = dnspod.load_domains()
    except Exception as e:
        dnspod.log('ERROR', '文件domains.json不存在或可能已经损坏')
        dnspod.exit_after_countdown()
    dnspod.update_ip()
    try:
    ...
    for d in domains:
        try:
            dnspod.log('INFO', '正在更新{}'.format(dnspod.sub_domain_and_domain(d.sub_domain, d.name)))
            ...
            if d.record_type not in qualified_record_types:
                fail.append(dnspod.sub_domain_and_domain(d.sub_domain, d.name))
            ...
            if d.value == record.get('value'):
                dnspod.log('INFO', '域名{}当前的记录未发生改变'.format(dnspod.sub_domain_and_domain(d.sub_domain, d.name)))
                fail.append(dnspod.sub_domain_and_domain(d.sub_domain, d.name))
                continue
            mx = record.get('mx')
            dnspod.modify_record(domain_id, record_id, sub_domain, record_type, record_line, value, mx,
                                 headers,
                                 params)
            success.append(dnspod.sub_domain_and_domain(d.sub_domain, d.name))
